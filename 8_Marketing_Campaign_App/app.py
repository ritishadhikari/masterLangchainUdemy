...
def getLLMResponse(query,ageOption,tasktypeOption,wordLengthOption):
    llm=OpenAI(
        temperature=0.9,
    ...
    elif ageOption=="Adult":  # Curious and Intelligent Adult
        examples=[
            {
            "query": "What is a mobile?",
    ...
    elif ageOption=="Senior Citizen": # A 90 years old guys
        examples=[
            {
            "query": "What is a mobile?",
    ...
    prefix="""
    You are a {templateAgeOption} and create a {templateTasktypeOption} in 
    {templateWordCounts}:
    Here are some examples:
    """

    suffix="""
    Question: {templateUserInput}
    Response:
    """
    ...
    newPromptTemplate=FewShotPromptTemplate(
    ...
    response=llm.invoke(
        input=newPromptTemplate.format(
            templateAgeOption=ageOption,
            templateTasktypeOption=tasktypeOption,
            templateUserInput=query,
            templateWordCounts=wordLengthOption
        )
    )   
    return response  
...
st.header(body="How Can I help You ?")
formInput=st.text_area(label="Enter text",height=275)

taskTypeOption=st.selectbox(
...
numberOfWords=st.slider(label="Words Limit",
                        min_value=1,
                        max_value=200,
                        value=25)
...
if submit:
    response=getLLMResponse(query=formInput,
                            ageOption=ageOption,
                            tasktypeOption=taskTypeOption,
                            wordLengthOption=str(numberOfWords)
                            
                            )
    st.subheader(body=response)
